...
import { CHARACTER_BY_ID_RESPONSE, SEARCH_AR_VALUES_RESPONSE, TEST_CAHRACTER_ID } from './mockItems';
export const handlers = [
  rest.get(`${API.host}/character`, (req, res, ctx) => {
    req.url.searchParams.set('name', '/^ar/i');
    return res(
      ctx.status(200),
      ctx.json(SEARCH_AR_VALUES_RESPONSE)
    );
  }),
  rest.get(`${API.host}/character`, (req, res, ctx) => {
    ...
    return res(
      ctx.status(200),
      ctx.json(CHARACTER_BY_ID_RESPONSE)
    );
  }),
