...
test("check name validation", () => {
  render(<NewCardForm newCardId={0} saveCard={() => {}} confirmAction={() => {}} />);
  fireEvent.input(screen.getByTestId("Name"),  { target: { value: mockNewCard.name.toLocaleLowerCase()} });
  fireEvent.submit(screen.getByRole("button"));
...
test("check startSell validation", () => {
  ...
  let tomorrow = new Date(curDate.setDate(curDate.getDate() - 1));
  render(<NewCardForm newCardId={0} saveCard={() => {}} confirmAction={() => {}} />);
  fireEvent.input(screen.getByTestId(/Start Sale/i),  { target: { value: tomorrow.toISOString().split('T')[0] }});
  fireEvent.submit(screen.getByRole("button"));
